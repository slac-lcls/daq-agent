...
Reports generated by `report` live in per-partition subdirectories inside a batch.
Latest-report discovery includes these directories; an explicit viewer path must
name the partition directory, not the parent batch. Prepared-only and failed
partitions are excluded. The viewer shows one partition at a time.
